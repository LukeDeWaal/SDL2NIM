...
def _process(process, simu=False, instance=False, taste=False, **kwargs):
    ...
    if not instance:
        ...
        start_transition = [
            ...
            *rand_reset_decl,
            ...
            if process.transitions else 'discard nil;',
            Init_Done,
            'Startup()']
    ...
    if instance:
        ...
        ads_template.append(
            f'procedure Execute_Transition (Id : Integer) renames {process.name}_Instance.Execute_Transition;')
    ...
    with open(process.name.lower() + os.extsep + 'ads', 'wb') as ada_file:
        ada_file.write(
            '\n'.join(format_nim_code(ads_template)).encode('latin1'))
...
def _floating_label(label, **kwargs):
    ...
    code.append(f'label {label.inputString}:')
    if label.transition:
